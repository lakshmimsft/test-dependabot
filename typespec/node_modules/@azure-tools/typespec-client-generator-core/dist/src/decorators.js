...
import { AllScopes, clientNameKey, getValidApiVersion, isAzureCoreModel, parseEmitterName, } from "./internal-utils.js";
...
export const $client = (context, target, options, scope) => {
    ...
    const service = explicitService?.kind === "Namespace"
        ? explicitService
        : (findClientService(context.program, target, scope) ?? target);
...
function findClientService(program, client, scope) {
    let current = client;
    while (current) {
...
export const $operationGroup = (context, target, scope) => {
    ...
    const service = findClientService(context.program, target, scope) ?? target;
    if (!isService(context.program, service)) {
        reportDiagnostic(context.program, {
            code: "client-service",
...
export function getOperationGroup(context, type) {
    let operationGroup;
    const service = findClientService(context.program, type, context.emitterName) ?? type;
    if (!isService(context.program, service)) {
        reportDiagnostic(context.program, {
            code: "client-service",
        ...
        if (params.filter((x) => compareModelProperties(x, value)).length === 0) {
            if (value.type.kind === "Model") {
                ...
                if (sourceProp.model && !isAzureCoreModel(sourceProp.model)) {
                    ...
                    console.log(`We are not counting "${sourceProp.name}" as part of a method parameter because it's been added by Azure.Core templates`);
                }
            }
        }
    });
...
export const paramAliasDecorator = (context, original, paramAlias, scope) => {
