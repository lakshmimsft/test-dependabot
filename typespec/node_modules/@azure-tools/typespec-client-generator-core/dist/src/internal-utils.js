    ...
    if (type.sourceModels.length === 1 && type.sourceModels[0].usage === "spread") {
        ...
        if (innerModel.name !== "" && innerModel.properties.size === type.properties.size) {
            return projectedProgram.projector
                ? projectedProgram.projector.projectedTypes.get(innerModel)
                : innerModel;
        }
        ...
        if (innerModel.sourceModels.length === 1 &&
            ...
            return projectedProgram.projector
                ? projectedProgram.projector.projectedTypes.get(innerModel.sourceModels[0].model)
                : innerModel.sourceModels[0].model;
        }
    }
    return type;
}
export function isOnClient(context, type, operation) {
