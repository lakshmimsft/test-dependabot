import { createDiagnosticCollector, getDoc, getSummary, ignoreDiagnostics, isErrorModel, } from "@typespec/compiler";
...
import { getAvailableApiVersions, getDocHelper, getHttpBodySpreadModel, getHttpOperationResponseHeaders, getLocationOfOperation, getTypeDecorators, isAcceptHeader, isContentTypeHeader, isHttpBodySpread, isNeverOrVoidType, isSubscriptionId, twoParamsEquivalent, } from "./internal-utils.js";
import { createDiagnostic } from "./lib.js";
import { getCrossLanguageDefinitionId, getEffectivePayloadType, isApiVersion, } from "./public-utils.js";
import { addEncodeInfo, getClientTypeWithDiagnostics, getSdkModelPropertyTypeBase, getTypeSpecBuiltInType, } from "./types.js";
...
function getSdkHttpParameters(context, httpOperation, methodParameters, responseBody) {
    ...
    if (tspBody) {
        ...
        if (tspBody.property && !isNeverOrVoidType(tspBody.property.type)) {
            ...
            let type;
            if (spread) {
                type = diagnostics.pipe(getClientTypeWithDiagnostics(context, getHttpBodySpreadModel(context, tspBody.type), httpOperation.operation));
            }
            else {
...
export function getSdkHttpParameter(context, param, operation, httpParam, location) {
    ...
    if (isPathParam(context.program, param) || location === "path") {
        ...
        const urlEncode = !ignoreDiagnostics(program.checker.isTypeAssignableTo(param.type.projectionBase ?? param.type, program.checker.getStdType("url"), param.type));
        return diagnostics.wrap({
            ...base,
            kind: "path",
            urlEncode,
            ...
            allowReserved: httpParam?.allowReserved ?? false,
...
export function getCorrespondingMethodParams(context, operation, methodParameters, serviceParam) {
    ...
    const correspondingClientParams = clientParams.filter((x) => twoParamsEquivalent(context, x.__raw, serviceParam.__raw) ||
