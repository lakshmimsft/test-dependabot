...
import { getAccessOverride, getOverriddenClientMethod, getUsageOverride, listClients, listOperationGroups, listOperationsInOperationGroup, shouldFlattenProperty, shouldGenerateConvenient, } from "./decorators.js";
import { UsageFlags, getKnownScalars, isSdkIntKind, } from "./interfaces.js";
import { createGeneratedName, filterApiVersionsInEnum, getAvailableApiVersions, getDocHelper, getHttpBodySpreadModel, getHttpOperationResponseHeaders, getLocationOfOperation, getNonNullOptions, getNullOption, getSdkTypeBaseHelper, getTypeDecorators, intOrFloat, isAzureCoreModel, isHttpBodySpread, isJsonContentType, isMultipartOperation, isNeverOrVoidType, isOnClient, isXmlContentType, twoParamsEquivalent, updateWithApiVersionInformation, } from "./internal-utils.js";
...
export function getSdkModelWithDiagnostics(context, type, operation) {
    const diagnostics = createDiagnosticCollector();
    let sdkType = context.modelsMap?.get(type);
    if (sdkType) {
        ...
        if (type.baseModel) {
            sdkType.baseModel = context.modelsMap?.get(type.baseModel);
...
export function getSdkUnionEnumWithDiagnostics(context, type, operation) {
    ...
    let sdkType = context.modelsMap?.get(union);
    if (!sdkType) {
...
function getKnownValuesEnum(context, type, operation) {
    const diagnostics = createDiagnosticCollector();
...
export function getClientTypeWithDiagnostics(context, type, operation) {
    ...
    switch (type.kind) {
        ...
        case "Union":
            ...
            break;
        case "ModelProperty":
            const innerType = diagnostics.pipe(getClientTypeWithDiagnostics(context, type.type, operation));
    ...
    if (operation && type.model) {
        ...
        if (httpBody) {
            const httpBodyType = isHttpBodySpread(httpBody)
                ? getHttpBodySpreadModel(context, httpBody.type)
                : httpBody.type;
...
function updateModelsMap(context, type, sdkType) {
    if (sdkType.kind !== "model" && sdkType.kind !== "enum") {
        return;
    }
    if (context.modelsMap === undefined) {
        context.modelsMap = new Map();
    }
...
function updateUsageOrAccessOfModel(context, value, type, options) {
    const diagnostics = createDiagnosticCollector();
    options = options ?? {};
    ...
    if (options?.seenModelNames === undefined) {
        options.seenModelNames = new Set();
    }
    if (type.kind === "model" && options.seenModelNames.has(type))
    ...
    if (type.additionalProperties) {
        options.ignoreSubTypeStack.push(false);
        diagnostics.pipe(updateUsageOrAccessOfModel(context, value, type.additionalProperties, options));
        options.ignoreSubTypeStack.pop();
    }
...
function updateTypesFromOperation(context, operation) {
    ...
    if (httpBody && !isNeverOrVoidType(httpBody.type)) {
        ...
        let sdkType;
        if (spread) {
            sdkType = diagnostics.pipe(getClientTypeWithDiagnostics(context, getHttpBodySpreadModel(context, httpBody.type), operation));
        }
        else {
        ...
        if (multipartOperation) {
            diagnostics.pipe(updateUsageOrAccessOfModel(context, UsageFlags.MultipartFormData, sdkType, {
                propagation: false,
            }));
        }
        const access = getAccessOverride(context, operation) ?? "public";
...
function updateAccessOverrideOfModel(context) {
    const diagnostics = createDiagnosticCollector();
    ...
    for (const sdkType of context.modelsMap?.values() ?? []) {
        const accessOverride = getAccessOverride(context, sdkType.__raw);
        if (accessOverride) {
            diagnostics.pipe(updateUsageOrAccessOfModel(context, accessOverride, sdkType, { isOverride: true }));
        }
    }
    return diagnostics.wrap(undefined);
}
function updateUsageOverrideOfModel(context) {
    const diagnostics = createDiagnosticCollector();
    for (const sdkType of context.modelsMap?.values() ?? []) {
        const usageOverride = getUsageOverride(context, sdkType.__raw);
        if (usageOverride) {
            diagnostics.pipe(updateUsageOrAccessOfModel(context, usageOverride, sdkType, { isOverride: true }));
        }
    }
    return diagnostics.wrap(undefined);
}
...
export function getAllModelsWithDiagnostics(context, options = {}) {
    ...
    for (const client of listClients(context)) {
        ...
        if (versionMap && versionMap.getVersions()[0]) {
            ...
            diagnostics.pipe(updateUsageOrAccessOfModel(context, UsageFlags.ApiVersionEnum, sdkVersionsEnum));
        }
    }
